...
int main(){
    ...
    double tempp = 0;
    double *tempv;
    tempv = (double*)malloc(__SIZEOF_DOUBLE__*features);


    //making copy of eigenvalues
    double *eigenvalcopy;
    eigenvalcopy = (double*)malloc(__SIZEOF_DOUBLE__*features);
    for(int g=0; g<features; g++){
    	eigenvalcopy[g]=eigenvalues[g];
    }

    //sorting the eigenvalues
    for(int vv = 0; vv < features; vv++){
    	for(int ww = 0; ww < features-1; ww++){

    		if(eigenvalues[ww+1]>eigenvalues[ww]){
    			
    			tempp = eigenvalues[ww+1];
    			eigenvalues[ww+1] = eigenvalues[ww];
    			eigenvalues[ww] = tempp;

    		}
    	}
    }

    //sorting the eigenvectors based on the eigenvalcopy
    for(int g=0; g<features; g++){
    	if(eigenvalues[g]!=eigenvalcopy[g]){

    		int ch = 0;
    		for(int h=0; h<features; h++){
    			if(eigenvalcopy[h]==eigenvalues[g]){
    				ch = h;
    				break;
    			}
    		}
    		...
    		for(int o=0; o<features; o++){
    			tempv[o] = eigenvectors[o][g];
    			eigenvectors[o][g] = eigenvectors[o][ch];
    			eigenvectors[o][ch] = tempv[o];
    		}
    	}
    }
